...
> 목표: hip-vinyl-v2 시안의 보미 캐릭터를 **리깅 + 4개 애니메이션 클립이 든 GLB 1개(< 3MB)** 로 만든다.
...
2. 아래 4개 클립을 **같은 캐릭터에** 각각 적용해 다운로드 (Format: FBX, Skin 포함은 첫 클립만, 이후는 Without Skin):

| 클립 | Mixamo 검색어 | 쓰이는 챕터 | 비고 |
|------|---------------|-------------|------|
| Run | "Running" (in place ✔) | CH1 PORTAL | In Place 체크 필수 — 이동은 코드가 담당 |
| Idle | "Breathing Idle" | CH2·CH3 정지·CH4·CH6 | 루프 자연스러운 것 |
...
1. Skin 포함 FBX 임포트 → 나머지 클립 FBX를 임포트해 **NLA 트랙으로 액션 병합** (액션 이름을 `Run / Idle / Walk / Jump`로 정리 — r3f에서 이 이름 그대로 씀).
...
- 검수: `npx @gltf-transform/cli inspect` 로 클립 4개·본·용량 확인 후 r3f `useAnimations` 연결 테스트.
...
- [ ] Mixamo 오토리깅 + 클립 4개 (Run/Idle/Walk/Jump, In Place 확인)
